...
#include "pico/rand.h"
/*
TITLE: PhoenixJack_serializeddatalogger
VERSION: 0.0.2
DATE: 2024-12-10
AUTHOR: CHRIS MCCREARY
TARGET HARDWARE: RASPBERRY PI PICO W
INTENT: Create a fast *NON-blocking way of saving data to SD card
ERRATA: 
  Technically, does block while doing a file write, BUT this is pretty quick
  There are more advanced options for true non-blocking file writes...
  Closes files after EVERY write to ensure data is saved
OVERVIEW:
  Starts a session with a (hopefully) unique id 0x____
  Saves data to sequential CSV files; rolls over at 100KB
  Maintains index file for all files for that session
  Maintains a time file which correlates the millis() counter with whatever epoch is given
